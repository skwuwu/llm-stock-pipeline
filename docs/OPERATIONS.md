...
> **왜 폐지 종목의 재무·주가는 안 받나:** 이 도구는 현재 시점 스크리너다.
> 현재 as_of 에는 생존편향이 0 이라(실측 2026-08-06 폐지 0종목) 필요가 없다.
> 과거 시점 백테스트를 하려면 필요하고, 무엇이 되고 안 되는지는
> [`ARCHITECTURE.md` §7.5](ARCHITECTURE.md) 에 실측으로 적었다.
