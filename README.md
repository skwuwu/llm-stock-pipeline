...
폐지 이력 2,093건을 수집하고 **사유를 주주 관점으로 분류**한다:
...
- **폐지 종목의 재무·주가는 아직 안 받았다.** 폐지 *이력*은 수집했지만
  (2,093건, 사유별 분류 포함) 그 종목들의 재무제표는 없다. 그래서 과거
  시점 스크린은 여전히 생존자만 본다 — 다만 이제 **그 크기를 알고 경고한다**
  (2023-01-01 기준 216종목). 소급 수집 경로는 열려 있다: 폐지 종목의
  `corp_code`가 DART corpCode 에 남아 있어 `ingest-dart --tickers` 로 받을 수 있다
