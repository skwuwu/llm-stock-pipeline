...
> ### 범위: 현재 시점 스크리닝 도구다 — 백테스터가 아니다
>
> "지금 무엇을 봐야 하는가"에 답하지, **"이 전략이 과거에 얼마를 벌었는가"에는
> 답하지 않는다.** 과거 날짜로 실행할 수는 있으나 그 결과를 성과 측정으로
> 쓰면 안 된다 — 이유는 §4에 수치로 적었다.
>
> PIT 설계(`available_at <= as_of`, 정정공시 append)는 백테스트를 위한 것이
> 아니라 **오늘의 스크린이 미래 정보를 쓰지 않게** 하려는 것이다. 어제 나온
> 재무제표를 오늘 쓰는 것과 3개월 뒤 정정될 값을 미리 쓰는 것은 다르다.
...
### 4. 생존편향을 재고, 그래서 범위를 좁혔다

유니버스는 KIND **현재** 상장사 목록에서 온다. 과거 시점으로 스크린을 돌리면
그때 상장돼 있다가 이후 폐지된 종목이 통째로 빠진다 — 살아남은 것만 남아
성과가 부풀려진다.

**현재 시점에는 이 편향이 없다.** 실측:

```
2026-08-06   상장 2,759 / 이후 폐지 0     → 편향 0. 지금 상장된 것을 보는 게 맞다
2023-01-01   상장 2,552 / 이후 폐지 216   → 8.5% 가 유니버스에 없다
```

**이것이 범위를 현재 시점으로 정한 이유다.** 과거 날짜 실행을 막지는 않지만
`screen` 이 편향 크기를 경고하고, 그 결과는 성과 측정에 쓸 수 없다.

폐지 이력 2,093건을 수집한 것은 **그 경고를 숫자로 만들기 위해서**다.
사유는 주주 관점으로 분류한다:

```
merged      합병·완전자회사화     대가를 받았다      120건 (2020+)
voluntary   자진상폐·공개매수     대개 프리미엄        12
dissolved   스팩 해산·존속만료    환급                86
failed      감사의견·계속성 결여   사실상 전손         177
other       분류 불가            failed 로 안 센다     9
```

**합병 소멸을 전손으로 처리하면 반대 방향으로 틀린다.** 더존비즈온(지주회사
완전자회사화)과 스타코링크(계속성 결여)는 같은 '폐지'가 아니다.

`screen`은 과거 as_of 에서 편향 크기를 경고하고, `survivorship`이 수치를 낸다:

```
2023-01-01 기준 상장 2,552종목 중 216종목이 이후 폐지 — 유니버스에 없다
...
| `ingest-delisting` | 상장폐지 이력 — **생존편향 해독제** | |
| `survivorship` | 생존편향 크기 측정 | |
...
- **과거 시점 실행은 지원 범위 밖이다.** 폐지 *이력*은 수집했지만(2,093건)
  그 종목들의 재무·주가는 받지 않았다. 현재 시점에는 편향이 0 이라 문제가
  없고, 과거 시점은 `screen` 이 경고한다. 범위를 넓히려면 무엇이 필요한지는
  [`ARCHITECTURE.md` §7.5](docs/ARCHITECTURE.md) 에 실측과 함께 적었다 —
  요약하면 재무·주가는 **받을 수 있고**(DART 3/5·FDR 8/8 확인), 관리종목
  지정 이력만 **소급이 불가능**하다
